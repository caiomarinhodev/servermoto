...
(function () {
    ...
    User = (function () {
        ...
        User.authenticate = function (siteId, userName, passWord, callback) {
            ...
            console.log('CHEGOU NO BACK -- usuario: '+userName+', senha: '+passWord);
            if (userName === 'anon') {
                user = {};
                user.site = siteId;
                user.id = '0';
            ...
            if (userName === 'anon') {
                ...
                async.waterfall([
                    function (cb) {
                        ...
                        var errMsg;
                        console.log("TRUE?: "+ hash === user.hash);
                        if (hash === user.hash) {
                            user.password = passWord;
                            cb(null, user);
                        } else {
        ...
        User.registerCustomer = function (req, res) { // Update userInfo
            ...
            console.log('----- VEIO DEVICE ID APP: ');
            ...
            async.waterfall([
                ...
                function (salt, hash, user, cb) {
                    ...
                    User.getUser(null, userName, function (err, result) {
                        console.log('----------- GET USER REGISTRO DE CLIENTE');
                        console.log(result);
                        if (!result) {
        ...
        User.registerDriver = function (req, res) {
            var password, userName, deviceId, company, driverType, fullname,
                taxiId, carLic, lat, lng, socketId, status;
        ...
        User.verifyToken = function (token, ip, cb) {
            ...
            if (!token || token == 'null') {
                ...
                console.log('TOKEN IS NULL OR VAZIO');
